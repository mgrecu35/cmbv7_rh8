...
//end    MG 10/29/15
	...
	for(j=0;j<88;j++)
	  {
	    if(isnan(asym[icount]))
	      asym[icount]=0;
	    kextSub[j][ifreq][i]=kext[icount];
	    salbSub[j][ifreq][i]=salb[icount];
	    asymSub[j][ifreq][i]=asym[icount];
	    if(isnan(asym[icount]))
	      {
		printf("%g %g %g %i %i\n",asym[icount],salb[icount],kext[icount],ifreq,j);
		printf("fmodel");
		exit(0);
	      }
	    icount++;
...
//  SFM  begin  07/29/2014; for M.Grecu  to eliminate NAN data
  ...
  for(ifreq=0;ifreq<8;ifreq++)
    for(j=0;j<88;j++)
      {
	kext[icount]=kextAvg[j][ifreq];
	salb[icount]=salbAvg[j][ifreq];
	asym[icount]=asymAvg[j][ifreq];
	icount++;
      }
//begin  MG 10/29/15
